...
let NewCombatText = CombatText['int NewText(Rectangle location, Color color, string text, bool dramatic, bool dot)'];
...
ChatCommandProcessor.ProcessIncomingMessage.hook((original, self, message, client_id) => {
  ...
  if (command.startsWith('/fortune ')) {
    ...
    if (!isNaN(fortuneValue)) {
      ...
      NewCombatText(Main.player[0].getRect(), Color.Blue, `Ores to be duplicated ${fortune}`, true, false);
    }
  }
});
